...
class User:
    ...
    def notify(self, text: str, silent: bool = False):
        ...
        Себя передаём как `menu_user`, чтобы под уведомлением сразу были актуальные
        кнопки действий (принять приглашение, подтвердить поимку и т.п.).
        """
        for ident in self.identities():
            if not ident.is_muted():
                ident.deliver(text, silent=silent, menu_user=self)
