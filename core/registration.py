...
def registration_allowed(user) -> bool:
    """Можно ли пользователю сейчас регистрироваться в игре.

    Разрешено, если открыта ОБЩАЯ регистрация ИЛИ у пользователя есть персональное
    приглашение в идущую игру (accept_invite): приглашение = разрешение пройти
    регистрацию, даже когда общая закрыта (TODO 94). Приглашение снимается по
    завершении регистрации (в `User.join`)."""
    return Game().is_registration_open() or user.has_game_invite()
