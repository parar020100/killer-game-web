...
def admin_management_buttons(user: User):
    ...
    # Порядок кнопок задаёт раскладку 2-колоночной сетки (.keyboard, row-major),
    # поэтому левый/правый столбцы чередуются в списке (TODO 80):
    #   ЛЕВО: Пауза · Открыть регистрацию · Завершить игру
    #   ПРАВО: Список игроков · Лог игры · Промежуточные итоги

    # 1Л) Жизненный цикл: один слот (старт / пауза / продолжить).
    if not started:
    ...
    # 1П) Показать/скрыть встроенный список игроков (переключатель на клиенте).
    b.append(_btn("👥 Список игроков", toggle="userlist"))

    # 2Л) Регистрация: закрыть, если открыта; иначе открыть — серая во время игры.
    if reg_open:
    ...
    # 2П) Показать/скрыть панель «Лог игры» (тот же механизм переключателя).
    b.append(_btn("📋 Лог игры", toggle="gamelog"))

    # 3Л) Завершение игры — серая вне паузы (опасное → подтверждение).
    ...
    # 3П) Промежуточные итоги — только пока игра идёт.
    ...
    # Далее — сброс и инструменты (продолжают сетку следующими строками).
...
def user_menu_buttons(target: User):
    ...
    # без паузы; при приёме входит «мёртвым» в случайной позиции (TODO 94). Повторно
    # приглашать можно (мог случайно отклонить) — кнопка просто меняет подпись.
    if is_player:
