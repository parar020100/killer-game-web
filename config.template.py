"""Шаблон настроек веб-версии «Киллер / Папарацци» — аналог config.template.py бота.

Скопируйте этот файл в `config.py` и подставьте свои значения:

    cp config.template.py config.py        # Linux/macOS/Git Bash
    copy config.template.py config.py      # Windows cmd

Сам `config.py` в `.gitignore` — он локальный и может содержать секреты.
Это простой образец: значения задаются прямо здесь (без чтения переменных
окружения), просто отредактируйте нужные строки.
"""

# --- инфраструктура ---------------------------------------------------------

# Путь к файлу БД (относительно папки проекта). Смена файла = другая «игра».
DB_NAME = "data/killer_game.db"

# Секрет подписи cookie-сессии. В ПРОДЕ ОБЯЗАТЕЛЬНО замените на длинную случайную
# строку (иначе сессии можно подделать). Сгенерировать:
#   python -c "import secrets; print(secrets.token_urlsafe(48))"
SECRET_KEY = "dev-insecure-secret-change-me"

# Границы валидации игрового имени.
NAME_MIN_LEN = 2
NAME_MAX_LEN = 50

# Базовый адрес сайта — из него боты собирают ссылку входа (/login?token=...).
# В проде укажите публичный адрес (напр. https://killer.example.com).
APP_BASE_URL = "http://127.0.0.1:8000"

# Длина случайной части токена входа (secrets.token_urlsafe).
LOGIN_TOKEN_BYTES = 32

# --- база данных ------------------------------------------------------------
# DB_BACKEND — движок БД: "sqlite" (по умолчанию, файл DB_NAME) или "postgres".
# Для postgres нужен пакет psycopg (см. setup/requirements.txt) и строка
# подключения POSTGRES_DSN. Переключение на PostgreSQL полезно для продакшена.
DB_BACKEND = "sqlite"
# DSN подключения к PostgreSQL, напр.: postgresql://user:pass@localhost:5432/killer
POSTGRES_DSN = ""

# --- админы по умолчанию (по платформам, раздельно) -------------------------
# Кто входит через указанный канал — сразу получает права администратора. Списки
# РАЗДЕЛЬНЫЕ для Telegram и VK: один и тот же человек может быть админом при входе
# через VK и не быть админом при входе через Telegram (и наоборот) — это разные
# аккаунты, пока их не объединили кодом привязки. Указывать можно числовой id ИЛИ
# username/screen_name канала (с «@» или без). Помимо этих списков всегда есть
# root-пользователь (создаётся при первом запуске, ссылка входа — в логе).
DEFAULT_ADMINS_TG = []          # напр. ["123456789", "my_tg_username"]
DEFAULT_ADMINS_VK = []          # напр. ["987654321", "my_vk_screenname"]

# --- Telegram-бот -----------------------------------------------------------
# ENABLE_TG_BOT — общий выключатель Telegram-канала (запуск бота + доставка).
ENABLE_TG_BOT = False
TELEGRAM_BOT_TOKEN = ""         # токен от @BotFather
TELEGRAM_BOT_USERNAME = ""      # напр. @my_game_bot (для ссылки t.me/<username>)

# --- VK-бот (сообщество) ----------------------------------------------------
# ENABLE_VK_BOT — общий выключатель VK-канала (запуск бота + доставка).
# Пошаговая настройка сообщества — в info/SETUP.md.
ENABLE_VK_BOT = False
VK_GROUP_TOKEN = ""
VK_GROUP_ID = ""               # число, id сообщества (без минуса)
VK_API_VERSION = "5.199"
VK_BOT_URL = ""                # напр. https://vk.com/my_game

# --- управление процессами (кнопки «перезагрузить / выключить / обновить») ---
# START_MODE — как запущено приложение (влияет на кнопки управления в настройках):
#   "manual"    — вручную из терминала (./start.sh / run_all.py);
#   "systemctl" — как сервис systemd (перезапуск/остановка через systemctl).
START_MODE = "manual"
SYSTEMD_SERVICE = "killer"      # имя unit-сервиса (при START_MODE = "systemctl")
CODE_DIR = ""                   # папка для `git pull` (пусто = папка проекта)

# Игровые настройки (контакт поддержки, файл правил, доп. вопросы, подтверждение
# поимок) хранятся в БД и правятся на странице «Настройки игры» (core/settings.py).
# Администраторов не задают в конфиге: при первом запуске создаётся root-
# пользователь, ссылка на вход для него печатается в лог — через него выдают права.
